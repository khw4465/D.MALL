        ...
        <div class="paging-container">
            ...
            <c:if test="${totalCnt!=null && totalCnt!=0}">
                ...
                <c:if test="${ph.showPrev}">
                    <a class="spacing"
                       href="<c:url value='/point/custSelectAll?page=${ph.beginPage-1}&pageSize=${ph.pageSize}'/>">&lt;</a>
                </c:if>
                <c:forEach var="i" begin="${ph.beginPage}" end="${ph.endPage}">
                    <a class="spacing"
                       href="<c:url value='/point/custSelectAll?page=${i}&pageSize=${ph.pageSize}'/>">${i}</a>
                </c:forEach>
                <c:if test="${ph.showNext}">
                    <a class="spacing"
                       href="<c:url value='/point/custSelectAll?page=${ph.endPage+1}&pageSize=${ph.pageSize}'/>">&gt;</a>
                </c:if>
            </c:if>
        </div>
