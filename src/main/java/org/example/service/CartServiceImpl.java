...
public class CartServiceImpl implements CartService {
    ...
    public int addCart(String custId, List<CartOptDto> list) throws Exception {
        ...
        CartDto dto = cartOptDao.summeryOpt(custId, prodCd);    // cartOpt에 담긴 해당 상품의 데이터를 한줄로 묶어준다.
    ...
    public int removeOpt(String custId, String prodCd, String optCd) throws Exception {     // 상품의 옵션 하나만 삭제하는 메서드
        ...
        CartDto cartDto = cartOptDao.summeryOpt(custId, prodCd);      // 한 상품안에 들은 옵션을 다시 요약
    ...
    @Override
    public int removeProd(String custId, String prodCd) throws Exception{
        Map<String, String> map = new HashMap<>();      // 맵에 담아서 전달
        map.put("custId", custId);
        map.put("prodCd", prodCd);
        cartOptDao.deleteProd(map);     // Cart_Opt 테이블에서 그 상품에 관한 모든 옵션목록 삭제
        return cartDao.delete(map);     // Cart에서 상품 삭제
    }
    @Override
...
//           throw new Exception("test");
        ...
        CartDto cartDto = cartOptDao.summeryOpt(custId, prodCd);
